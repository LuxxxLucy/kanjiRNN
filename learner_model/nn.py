...
def get_length(sequence,false_value=-1):
    '''
    compute the length (for recurrent neural network), when the length is not fixed.

    false_value is set for the padding. If zero padding, the set it to zero, if -1-padding, set it to -1
    '''
    used = tf.sign(sequence-false_value)
